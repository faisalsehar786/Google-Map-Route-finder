...
class GMaps
{
    ...
    public function add_marker($params = array())
    {
        ...
        if ($marker['infowindow_content'] != "") {
            ...
            $marker_output .= '
            marker_'.$marker_id.'.set("content", "'.trim($marker['infowindow_content']).'");
           
            google.maps.event.addListener(marker_'.$marker_id.', "click", function(event) {

                makerOptionsModal(this.get("content"));
            map.setZoom(13);
  ...
  var lineSymbol = {
    ...
    strokeOpacity: 1
            ...
            var directionsDisplay = new google.maps.DirectionsRenderer({ polylineOptions: { strokeColor: "#EC1B23",
    strokeOpacity: 0.8,
    strokeWeight: 7,
    icons: [{
            icon: lineSymbol,
            ...
            if ($this->placesAutocompleteInputID != "") {
                ...
                if ($this->placesAutocompleteOnChange == "") {
                    $this->output_js_contents .= 'google.maps.event.addListener(placesAutocomplete, \'place_changed\', function() {
                        ...
                        var searchlocation=$("#myPlaceTextBox").val();
                        $("#routsearchLocation").val(searchlocation);
        ...
        if ($this->directions && $this->directionsStart != "" && $this->directionsEnd != "") {
            if ($this->directionsStart == "auto" && $this->directionsEnd == "auto") {
                ...
                if(navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                        start = position.coords.latitude+","+position.coords.longitude;
                        calcRoute(start, \''.trim($this->directionsEnd).'\');
                    }, function() { alert("Unable to get your current position. Please try again. Geolocation service failed."); });
                // Browser doesn\'t support Geolocation
                }else{
                    alert(\'Your browser does not support geolocation.\');
                ...
                if(navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        calcRoute(\''.trim($this->directionsStart).'\', end);
                    }, function() { alert("Unable to get your current position. Please try again. Geolocation service failed."); });
                // Browser doesn\'t support Geolocation
                }else{
                    alert(\'Your browser does not support geolocation.\');
                ...
                $this->output_js_contents .= '
                calcRoute(\''.trim($this->directionsStart).'\', \''.trim($this->directionsEnd).'\');
                ';
            }
        }
        ...
        if ($this->directions) {
            ...
            $this->output_js_contents .= 'function calcRoute(start, end) {';
                ...
                foreach ($this->directionsWaypointArray as $waypoint) {
                    ...
                    $directionsWaypointStr .= '{ location: "'.trim($waypoint).'", stopover: true}';
                }
                $this->output_js_contents .= '  var downloadRoutedata=[]; var stations=['.$directionsWaypointStr.'];';

                 $this->output_js_contents.='for (var i = 0, parts = [], max = 25 - 1; i < stations.length; i = i + max){
    ...
    $this->output_js_contents .='   for (var i = 0; i < parts.length; i++) {
                var waypoints = [];
            ...
            $this->output_js_contents .= '
                ...
                directionsService.route(request, function(response, status) {
                    if (status == google.maps.DirectionsStatus.OK) {
                         ...
                         directionsDisplay = new google.maps.DirectionsRenderer({ polylineOptions: { strokeColor: "#EC1B23",strokeOpacity: 0.8,
                                                strokeWeight: 7,
                                                 icons: [{
                                                  icon: lineSymbol,
                                                  offset: "100px",
                        ...
                        directionsDisplay.setPanel(document.getElementById("'.$this->directionsDivID.'"));

                         $(".loader").show();

                       
                               
                    var getrouteDataObject=response.routes[0].legs[0].via_waypoints;
                          
                    var locationName= response.request;
                        
                      
                   

                           var startenddestination=[{"startpointAddress": response.routes[0].legs[0].start_address ,"startpointAddreslatilong":response.routes[0].legs[0].start_location.toJSON(),"endpointAddress": response.routes[0].legs[0].end_address , "endpointAddreslatilong":response.routes[0].legs[0].end_location.toJSON()}];
                     
                        var dfind=0;


                           for (var iz = 0; iz < getrouteDataObject.length   ; iz++) {

                            if (iz === getrouteDataObject.length - 1) {
                               break;
                               }
                            
                                dfind=dfind+1;
                                   
                     downloadRoutedata.push({"start_way_address":locationName.waypoints[iz].location.query,"start_lat_long_way":getrouteDataObject[iz].toJSON(),"end_way_address":locationName.waypoints[dfind].location.query,"end_lat_long_way":getrouteDataObject[dfind].toJSON()});


                           }

                           
                        dataString = downloadRoutedata; // array?

                     desinationstartendString=startenddestination;
                         var jsonString = JSON.stringify(dataString);
                          var desinationstartendjsonString = JSON.stringify(desinationstartendString);

                        
                           $.ajax({
                            type: "POST",
                            url: "'.url('preparedownloadExcel').'",
                            data:{dataWaypoints:jsonString,startEndDes:desinationstartendjsonString,_token:"'. csrf_token().'"}, 
                            cache: false,
                           ...
                           success: function(res){
                            $(".loader").fadeOut(3000);
                               ...
                               error: function(XMLHttpRequest, textStatus, errorThrown) { 
                                   ...
                                   $(".loader").fadeOut(3000);
                                    toastr.error("Please Try Again Download File is Not Ready");
                                }    
    ...
    public function get_lat_long_from_address($address, $attempts = 0)
    {
        ...
        $cleanStrAddress = preg_replace('/[^A-Za-z0-9 ]/', '', $address);
        $data_location = "https://maps.google.com/maps/api/geocode/json?address=".urlencode(utf8_encode($cleanStrAddress ))."&key=".config('googlemaps.key')."&sensor=".$this->sensor;
        if ($this->region != "" && strlen($this->region) == 2) {
